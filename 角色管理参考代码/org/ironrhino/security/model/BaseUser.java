...
public class BaseUser extends BaseRecordableEntity implements RoledUserDetails, Enableable {
	...
	@UiConfig(displayOrder = 100, alias = "role",excludedFromCriteria=true, template = "<#list value as r>${statics['org.ironrhino.core.util.ApplicationContextUtils'].getBean('userRoleManager').displayRole(r)}<#if r_has_next> </#if></#list>", csvTemplate = "<#list value as r>${statics['org.ironrhino.core.util.ApplicationContextUtils'].getBean('userRoleManager').displayRole(r)}<#if r_has_next>,</#if></#list>")
