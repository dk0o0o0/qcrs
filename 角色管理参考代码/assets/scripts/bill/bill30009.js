...
function typeChange(){
	if ($("#billType").val()=="1"){
		...
		$("#ecdsConveyType").attr("disabled",true);
		$("#redeemRate").attr("readonly",true);
		$("#redeemOpenDate").attr("disabled",true);
		$("#startInterDate").attr("disabled",true);
		$("#maturityDate").attr("disabled",true);
		...
		$("#startInterDate").removeAttr("disabled");
...
function verifyStartInterDate(){
	if(!validateMaturityDate($("#maturityDate").val(),$("#startInterDate").val())){
		...
		//如果日期出错，则都赋起息日
		$("#maturityDate").val($("#startInterDay").val());
		$("#startInterDate").val($("#startInterDay").val());
	}else{
		count();
	}
}
...
// -------------------------table//查看按钮事件
//function viewTrData30009_10(thisObj) {
//	//获取查看数据
...
//	var url = "../bussiness/billPublic/pjShow?busiType="
//			+ busiType;
//
...
$(document).ready(function() {
	...
	$(document).on('click','#btn_import',function() {
		//导入前判断是否已经数据利率，否则无法计算成交金额，利息
			//if (transDirection != null&& transDirection.selectedIndex == 0) {
			if ($("#billRate").length > 0 && $("#billRate").val() == 0) {
				doTheAlert('提示',"请输入利率！");
				$("#billRate").focus();
				return false;
			//}
...
function count(){
	...
	for (var j=0;j<i;j++){
		...
		var val = $("#bill_cntr30009 [name='"+selectorName+"']");
		if (i==1){
			if($(val)[0].value==""){
				stop=true
				break;
...
//		val = $("#bill_cntr30009 [name='"+fieldName+"']");
		
		//第I行到期日
		selectorName = 'billCntr.items['+ j +'].billNote.matureDate';
		val = $("#bill_cntr30009 [name='"+selectorName+"']");
		...
		if (billType=="0"){
			...
			var val = $("#table [name='"+selectorName+"']");
			redeemAmountSum+=parseFloat(getMoneyValue($(val)[0].value));
		}
