...
function count(){
	...
	for (var j=0;j<i;j++){
		...
		var val = $("#bill_cntr30004 [name='"+selectorName+"']");
		if (i==1){
			if($(val)[0].value==""){
				stop=true;
				break;
		...
		val = $("#bill_cntr30004 [name='"+fieldName+"']");
		...
		if (billType=="0" && ($("#supplyDays").val()=="1")){
			if (region=='异地')
				$(val)[0].value='3';
			else 
				$(val)[0].value='0';
		...
		val = $("#bill_cntr30004 [name='"+selectorName+"']");
